...
const Header = () => {
  ...
  <div className="navbar-center hidden lg:flex text-yellow-600  font-bold items-center ">
    <ul className="menu menu-horizontal space-x-4 px-1 text-yellow-600 items-center  font-bold ">
     <Link to="/menu" className='text-xl '>Our Menu</Link>
     
     <Link to="/order/salad" className='text-xl '> Order Food </Link>
  ...
  <img className='w-[50px] h-[50px] rounded' src={user?.photoURL} alt={user?.displayName} />
  ...
  <Link to="/dashboard/mycart" className='text-xl text-white'> 
    ...
    <div className="badge badge-secondary">+{cart?.length || 0}</div>
      ...
      <button onClick={handlelogout} className='btn btn-warning  text-xl'>Log Out</button> </> :
      //if not availble
       <>
